...
<div class="org-row">
  <!-- Main Node -->
  <div class="org-node-horizontal">
    <div class="org-node-content">
      @if($unit->photo)
        <img src="{{ asset('storage/' . $unit->photo) }}" class="org-photo" alt="">
      @else
  ...
  <!-- Children (stacked vertically on the right) -->
  @if($unit->children->count() > 0)
    <div class="org-children-horizontal">
      @foreach($unit->children as $child)
        <x-org-unit-horizontal :unit="$child" />
      @endforeach
    </div>
  @endif
</div>
