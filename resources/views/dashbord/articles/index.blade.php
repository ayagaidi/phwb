...
@section('title', __('admin.app_name') . ' | ' . __('admin.articles.management'))
@section('page-title', __('admin.articles.title'))
...
<div class="stats-cards-row mb-4">
  <div class="stat-card-users">
    ...
    <p class="lbl">{{ __('admin.articles.total_articles') }}</p>
  ...
  <div class="stat-card-users">
    ...
    <p class="lbl">{{ __('admin.articles.published') }}</p>
  ...
  <div class="stat-card-users">
    ...
    <p class="lbl">{{ __('admin.articles.drafts') }}</p>
...
<div class="table-card">
  <div class="table-card-hdr">
    <h3 class="table-card-title">{{ __('admin.articles.articles_list') }}</h3>
  ...
  <div class="table-responsive">
    <table class="data-table">
      <thead>
        <tr>
          <th>#</th>
          <th>{{ __('admin.articles.title_label') }}</th>
          <th class="tc">{{ __('admin.articles.status') }}</th>
          <th class="tc">{{ __('admin.articles.date') }}</th>
          <th class="tl">{{ __('admin.articles.actions') }}</th>
        </tr>
      </thead>
      <tbody>
          ...
          <tr>
            ...
            <td class="tc">
              @if($article->is_published)
                <span class="badge badge-green">{{ __('admin.articles.published_badge') }}</span>
              @else
                <span class="badge badge-gray">{{ __('admin.articles.draft_badge') }}</span>
              @endif
            </td>
            <td class="tc">{{ $article->created_at ? $article->created_at->format('Y-m-d') : '-' }}</td>
            <td>
              <div class="action-btns">
                <a href="{{ route('admin.articles.edit', $article->id) }}" class="icon-btn icon-btn-edit" title="{{ __('admin.articles.edit') }}">
                ...
                <form method="POST" action="{{ route('admin.articles.toggle', $article->id) }}" style="display:inline;">
                  @csrf @method('PATCH')
                  <button type="submit" class="icon-btn {{ $article->is_published ? 'icon-btn-reject' : 'icon-btn-accept' }}" title="{{ $article->is_published ? __('admin.articles.unpublish') : __('admin.articles.publish') }}">
                    @if($article->is_published)
                   ...
                   @csrf @method('DELETE')
                   <button type="button" class="icon-btn icon-btn-reject" onclick="confirmDelete({{ $article->id }})" title="{{ __('admin.articles.edit') }}">
...
function confirmDelete(id) {
  Swal.fire({
    ...
    text: '{{ __("admin.confirm.delete_article") }}',
    icon: 'warning',
