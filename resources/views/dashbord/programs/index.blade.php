...
@section('title', 'صيادلة بلا حدود | البرامج')
@section('page-title', 'إدارة البرامج')
...
<div class="table-card">
  <div class="table-card-hdr">
    <h3 class="table-card-title">قائمة البرامج</h3>
  ...
  <div class="table-responsive">
    <table class="data-table">
      ...
      <tbody>
            ...
            <td>
              <div class="cell-name">
                ...
                <div>
                  ...
                  <small>{{ Str::limit($program->description, 50) }}</small>
                </div>
              </div>
            </td>
            <td class="tc">
              ...
              @else
                <span class="badge badge-gray">غير منشور</span>
              @endif
            </td>
            <td>
              <div class="action-btns">
                <a href="{{ route('admin.programs.edit', $program->id) }}" class="icon-btn icon-btn-edit" title="تعديل">
                ...
                <form method="POST" action="{{ route('admin.programs.toggle', $program->id) }}" style="display:inline;">
                  @csrf @method('PATCH')
                  <button type="submit" class="icon-btn {{ $program->is_published ? 'icon-btn-accept' : 'icon-btn-reject' }}" title="{{ $program->is_published ? 'إلغاء النشر' : 'نشر' }}">
                    @if($program->is_published)
        ...
        @empty
          <tr>
            <td colspan="4" style="text-align:center; padding:3rem; color:var(--muted);">لا توجد برامج بعد</td>
          </tr>
        @endforelse
      </tbody>
    </table>
