...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans+Arabic:wght@400;500;600;700;900&display=swap"
        rel="stylesheet" />
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="{{ route('admin.permissions') }}"
                    class="nav-item {{ request()->routeIs('admin.users*') || request()->routeIs('admin.permissions*') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2"
                        fill="none">
                        <path d="M12 15v2m-6 4h12a2 2 0 0 0 2-2v-6a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2z"/>
                        <path d="M12 9a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
                    </svg>
                    <span class="nav-label">{{ __('admin.nav.permissions') }}</span>
                </a>
                @endif
                ...
                <a href="{{ route('admin.sliders') }}"
                    ...
                    <span class="nav-label">{{ __('admin.nav.sliders') }}</span>
                </a>
                @endif
                @if(hasPermission('volunteer-content', 'view'))
                <a href="{{ route('admin.volunteer-content') }}"
                    class="nav-item {{ request()->routeIs('admin.volunteer-content*') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2"
                        fill="none">
                ...
                @endif
                @if(hasPermission('membership-applications', 'view'))
                <a href="{{ route('admin.membership-applications') }}"
                    class="nav-item {{ request()->routeIs('admin.membership-applications*') ? 'active' : '' }}">
                    <i class="nav-icon fas fa-user-plus"></i>
                    <span class="nav-label">{{ __('admin.nav.membership_applications') }}</span>
                </a>
                @endif
                @if(hasPermission('donation-methods', 'view'))
                <a href="{{ route('admin.donation-methods') }}"
                    class="nav-item {{ request()->routeIs('admin.donation-methods*') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2"
                        fill="none">
                        <path d="M12 2v20m10-10H2" />
                    </svg>
                ...
                @endif
                @if(hasPermission('org-structure', 'view'))
                <a href="{{ route('admin.org-structure') }}"
                    class="nav-item {{ request()->routeIs('admin.org-structure*') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2"
                        fill="none">
                ...
                @endif
                @if(hasPermission('contact-settings', 'view'))
            ...
            <header class="toolbar">
                ...
                <div class="toolbar-actions">
                    ...
                    <div class="lang-switcher">
                        <a href="{{ route('admin.lang', 'ar') }}"
                            class="lang-btn {{ app()->getLocale() === 'ar' ? 'active' : '' }}"
                            title="{{ __('admin.arabic') }}">AR</a>
                        <a href="{{ route('admin.lang', 'en') }}"
                            class="lang-btn {{ app()->getLocale() === 'en' ? 'active' : '' }}"
                            title="{{ __('admin.english') }}">EN</a>
                    </div>
                    ...
                    <div class="toolbar-notifications dropdown">
                        ...
                        <button class="toolbar-icon-btn" type="button"
                            aria-label="{{ __('admin.toolbar.notifications') }}">
                        ...
                        <div class="dropdown-menu"
                            style="right:auto; min-width:300px; max-width:340px; padding:4px 0;">
                            <div class="dropdown-header"
                                style="display:flex; justify-content:space-between; align-items:center; padding:8px 14px;">
                                <strong>الإشعارات</strong>
                                @if ($pendingCount > 0)
                                    <span
                                        style="font-size:11px; background:#fee2e2; color:#b91c1c; padding:1px 6px; border-radius:999px; font-weight:600;">{{ $pendingCount }}
                                        جديد</span>
                                @endif
                            </div>
                            ...
                            @forelse($recentPending as $app)
                                <div class="dropdown-item"
                                    style="display:flex; align-items:center; justify-content:space-between; padding:8px 14px; gap:8px;">
                                    <a href="{{ route('admin.membership-applications.show', $app->id) }}"
                                        ...
                                        <div style="font-size:11.5px; color:#64748b; margin-top:1px;">
                                            {{ str_replace('_', ' ', $app->membership_type) }} •
                                            {{ $app->created_at->diffForHumans() }}
                                        </div>
                                    </a>
                                    <form method="POST"
                                        action="{{ route('admin.membership-applications.mark-read', $app->id) }}"
                                        style="margin:0;">
                                        @csrf
                                        <button type="submit" title="وضع كمقروء"
                            ...
                            @if ($pendingCount > 0)
                                ...
                                <form method="POST"
                                    action="{{ route('admin.membership-applications.mark-all-read') }}"
                                    style="margin:0;">
                                    @csrf
                                    <button type="submit" class="dropdown-item"
                                        style="font-weight:600; color:#b91c1c; width:100%; text-align:left; border:none; background:none; cursor:pointer; padding:8px 14px;">
                                        وضع الكل كمقروء
                                    </button>
                                </form>

                                <a href="{{ route('admin.membership-applications') }}" class="dropdown-item"
                                    style="font-weight:600; color:var(--accent); padding:8px 14px;">
                    ...
                    <div class="toolbar-avatar dropdown" aria-hidden="true">
                        <button class="toolbar-avatar-btn" type="button"
                            aria-label="{{ __('admin.toolbar.profile') }}">
                        ...
                        <div class="dropdown-menu">
                            ...
                            <div class="dropdown-divider"></div>
                            <button style="background-color: #fff; border: 1px #fff;" type="button"
                                class="dropdown-item text-red"
                                onclick="document.getElementById('dropdown-logout-form').submit();">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                                    stroke="currentColor" stroke-width="2">
                    ...
                    <form id="dropdown-logout-form" action="{{ route('admin.logout') }}" method="POST"
                        style="display: none;">
                        @csrf
                    </form>
                </div>
